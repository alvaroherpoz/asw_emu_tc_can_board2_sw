											...
											, CEDROOMMemoryPool *pDataPool)
{
	...
	if (m_IsSenderRemote){
		//Reply to remote
		mp_Interface->mp_Channel->reply_to_remote(signal,dataObjetPtr,pDataPool);
		mp_MsgBack=NULL;

	}else if(m_IsSenderWaiting){
		// Complete Msg
...
//*******************************************************************************************************
//************************************CEDROOMInterface::invoke_from_remote *******************************

void CEDROOMInterface::invoke_from_remote(TEDROOMSignal signal
									, void *pData, CEDROOMMemoryPool * pDataPool){


	mp_Channel->invoke_from_remote(signal,pData,pDataPool);

}


//*******************************************************************************************************
//************************************CEDROOMInterface::reply_from_remote *******************************

void CEDROOMInterface::reply_from_remote(TEDROOMSignal signal
									, void *pData, CEDROOMMemoryPool * pDataPool){

	mp_Channel->reply_from_remote(signal,pData, pDataPool);
...
void CEDROOMInterface::reply_from_remote(TEDROOMSignal signal)
{
	mp_Channel->reply_from_remote(signal);
}
...
void CEDROOMLocalTXChannel::invoke_from_remote(TEDROOMSignal signal
									, void *p_data
									, CEDROOMMemoryPool * p_DataPool){
...
void CEDROOMLocalTXChannel::reply_to_remote(TEDROOMSignal signal,void * p_data
		,CEDROOMMemoryPool *p_DataPool){
...
void CEDROOMLocalTXChannel::reply_from_remote(TEDROOMSignal signal)
{
	if(mp_SynchrMsgFromRemote)
	{
		mp_SynchrMsgFromRemote->reply(signal);

		mp_SynchrMsgFromRemote=NULL; //Only one reply
	}
}
void CEDROOMLocalTXChannel::reply_from_remote(TEDROOMSignal signal,void * p_data
		,CEDROOMMemoryPool *p_DataPool){

	if(mp_SynchrMsgFromRemote) {
		mp_SynchrMsgFromRemote->reply(signal,p_data,p_DataPool);
