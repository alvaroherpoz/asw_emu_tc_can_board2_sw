...
/*!
 * \class CEDROOMRemoteSynchrMessage
 * \brief class that implements the synchronous messages.
 * \author Oscar Rodriguez Polo
 */
...
class CEDROOMRemoteSynchrMessage:public CEDROOMMessage
{
	...
	class CEDROOMRemoteTXChannel * m_remoteChannel;
...
public:
	...
	*/
	...
	CEDROOMRemoteSynchrMessage(){ m_remoteChannel=NULL;};
...
class CEDROOMInterface{
	...
	friend class CEDROOMSystemCommSAP;
	friend class CEDROOMTimingSAP;
	friend class CEDROOMLocalTXChannel;
	friend class CEDROOMRemoteTXChannel;
...
public:
    ...
    */
	...
	/*!
	    * \brief forwarding a synchronous invoke received from a remote component
	    * \param signal message identifier
	    * \param pData pointer to the attached data
	    * \param pDataPool pointer to the attached data pool
	    */
	void invoke_from_remote(TEDROOMSignal signal
									, void *pData, CEDROOMMemoryPool * pDataPool);

		/*!
		    * \brief forwarding a reply from a remote component
		    * \param signal message identifier
		    * \param pData pointer to the attached data
		    * \param pDataPool pointer to the attached data pool
		    * \return the reply message
		    */
	void reply_from_remote(TEDROOMSignal signal
										, void *pData, CEDROOMMemoryPool * pDataPool);
...
public:
    ...
    */
	...
	/*!
	    ...
	    * \param pDataPool pointer to the attached data pool
	    */
	virtual void invoke_from_remote(TEDROOMSignal signal
									, void *pData, CEDROOMMemoryPool * pDataPool)=0;

		/*!
		    * \brief forwarding a reply  component
		    * \param signal message identifier
		    * \param pData pointer to the attached data
		    * \param pDataPool pointer to the attached data pool
		    */
	virtual void reply_to_remote(TEDROOMSignal signal
										, void *pData, CEDROOMMemoryPool * pDataPool)=0;



	/*!
	    * \brief Manage the reply from remote component
	    * \param signal message identifier
	    * \param pData pointer to the attached data
	    * \param pDataPool pointer to the attached data pool
	    */
	virtual void reply_from_remote(TEDROOMSignal signal
									, void *pData, CEDROOMMemoryPool * pDataPool)=0;
	...
	virtual void reply_from_remote(TEDROOMSignal signal)=0;
};
...
protected:
  ...
  public:
	...
	//! pointer to Synchronous Message From Remote
	CEDROOMSynchrMessage *mp_SynchrMsgFromRemote;
...
public:
    ...
    */
	...
	/*!
	    * \brief reply to a remote component
	    * \param signal message identifier
	    * \param pData pointer to the attached data
	    * \param pDataPool pointer to the attached data pool
	    * \return the reply message
	    */
	void reply_to_remote(TEDROOMSignal signal
									, void *pData, CEDROOMMemoryPool * pDataPool);
 ...
 * \brief This class implements the EDROOM remote connection channels. It must be implemented in each edroomdeployment\
          according to the protocol of the specific remote communication channel
 *
 * \author Oscar Rodriguez Polo
 */
//******************************************************************************

class CEDROOMRemoteTXChannel: public CEDROOMLocalTXChannel{
